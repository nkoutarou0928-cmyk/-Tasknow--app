...
export const DashboardView: React.FC = () => {
  ...
  const getDeadlineBadge = (remainingHours: number) => {
    if (remainingHours <= 0) {
      return { className: 'badge-urgent', text: '期限切れ' };
    } else if (remainingHours < 24) {
      return { className: 'badge-urgent', text: `残り ${Math.round(remainingHours)}時間` };
    } else if (remainingHours < 72) {
      return { className: 'badge-warning', text: `残り ${Math.round(remainingHours / 24)}日` };
    }
    return { className: 'badge-safe', text: `残り ${Math.round(remainingHours / 24)}日` };
  };
            ...
            >
              {sortedActiveTasks.length === 0 ? (
                ...
                sortedActiveTasks.map((task, index) => {
                  ...
                  const badge = getDeadlineBadge(remainingHours);
                    ...
                    >
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`task-card-outer ${isYabai ? 'yabai-glow' : ''}`}
                          style={{
                            display: 'flex',
                            alignItems: 'center',
                            padding: '16px',
